using System.Web.Optimization;

namespace Website
{
    public class BundleConfig
    {
        // For more information on bundling, visit http://go.microsoft.com/fwlink/?LinkId=301862
        public static void RegisterBundles(BundleCollection bundles)
        {
            bundles.Add(new ScriptBundle("~/bundles/jquery").Include(
                        "~/Scripts/jquery-{version}.js"));

            bundles.Add(new ScriptBundle("~/bundles/jqueryval").Include(
                        "~/Scripts/jquery.validate*"));

            // Use the development version of Modernizr to develop with and learn from. Then, when you're
            // ready for production, use the build tool at http://modernizr.com to pick only the tests you need.
            bundles.Add(new ScriptBundle("~/bundles/modernizr").Include(
                        "~/Scripts/modernizr-*"));

            bundles.Add(new ScriptBundle("~/bundles/bootstrap").Include(
                      "~/Scripts/bootstrap.js",
                      "~/Scripts/respond.js"));

            bundles.Add(new ScriptBundle("~/bundles/angular").Include(
                       "~/Scripts/Vendor/Angular/angular.js",
                       "~/Scripts/Vendor/Angular/angular-route.js",
                       "~/Scripts/Vendor/Angular/angular-cookies.js"));

            bundles.Add(new ScriptBundle("~/bundles/angularServices").Include(
                                            "~/Scripts/App/application.js",
                                            "~/Scripts/App/serverApi.js",
                                            "~/Scripts/App/services/authenticationService.js"));
            
            bundles.Add(new ScriptBundle("~/bundles/angularRepos").Include(
        "~/Scripts/App/repositories/accountRepository.js"));

            bundles.Add(new ScriptBundle("~/bundles/angularCtrls").Include(
                "~/Scripts/App/controllers/accountCtrl.js",
                "~/Scripts/App/controllers/homeController.js",
                "~/Scripts/App/controllers/loginController.js",
                "~/Scripts/App/controllers/registerController.js"));

            

            bundles.Add(new StyleBundle("~/Content/css").Include(
                      "~/Content/bootstrap.css",
                      "~/Content/site.css"));
        }
    }
}
